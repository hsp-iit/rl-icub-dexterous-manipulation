...
from rl_icub_dexterous_manipulation.external.d3rlpy_mod.d3rlpy.online.buffers import Buffer
...
def train_single_env(
    ...
    if type(env.observation_space) is gym.spaces.Dict:
        observation_shape_len = 0
        for obs_key in env.observation_space.keys():
            observation_shape_len += max(env.observation_space[obs_key].shape)
        observation_shape = (observation_shape_len,)
    else:
        ...
        with logger.measure_time("step"):
            ...
            if is_image:
                ...
                if type(observation) is dict:
                    observation_vec = np.empty(observation_shape[0],)
                    obs_start_id = 0
                    for obs_key in observation.keys():
                        obs_end_id = obs_start_id + observation[obs_key].squeeze().size
                        observation_vec[obs_start_id:obs_end_id] = observation[obs_key].squeeze()
                        obs_start_id = obs_end_id
                    observation = observation_vec.astype("f4")
                else:
