...
from rl_icub_dexterous_manipulation.external.stable_baselines3_mod.common.base_class import BaseAlgorithm
from stable_baselines3.common.buffers import DictReplayBuffer, ReplayBuffer
from stable_baselines3.common.callbacks import BaseCallback
from stable_baselines3.common.noise import ActionNoise, VectorizedActionNoise
from rl_icub_dexterous_manipulation.external.stable_baselines3_mod.common.policies import BasePolicy
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _sample_action(
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            if self.curriculum_learning and self.num_timesteps < self.curriculum_max_steps:
                curriculum_scale_factor = max(0.1, self.num_timesteps / self.curriculum_max_steps)
                curriculum_scale_factor_arr = np.ones(action.shape[1])
                curriculum_scale_factor_arr[self.curriculum_learning_components] = np.repeat(curriculum_scale_factor,
                                                                                             len(action))
                scaled_action = np.clip(scaled_action, -curriculum_scale_factor_arr, curriculum_scale_factor_arr)
                action = self.policy.unscale_action(scaled_action)
                buffer_action = scaled_action
        else:
