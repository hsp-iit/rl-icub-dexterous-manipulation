...
class ContinuousCritic(BaseModel):
    ...
    def __init__(
        ...
        compute_penultimate_layer_features: bool = False
    ):
        super().__init__(
            observation_space,
            action_space,
        ...
        self.feat_penultimate_layer = {}

        for idx in range(n_critics):
            q_net = create_mlp(features_dim + action_dim, 1, net_arch, activation_fn)
            q_net = nn.Sequential(*q_net)
            # Register hook to extract features from the penultimate layer, if required
            if compute_penultimate_layer_features:
                q_net[-2].register_forward_hook(self.get_feat(f"feat{idx}"))
            self.add_module(f"qf{idx}", q_net)
            self.q_networks.append(q_net)

    # Define function to extract features from the penultimate layer
    def get_feat(self, name):
        def hook(model, input, output):
            self.feat_penultimate_layer[name] = output
        return hook
